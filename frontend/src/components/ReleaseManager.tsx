...
export function ReleaseManager({
  ...
  function activate(node: LineageNode) {
    ...
    if (node.is_baseline) {
      toast("Baselines load read-only — coming in a later slice.");
      return;
    }
                ...
                {visible.map((n) => (
                  ...
                  >
                    ...
                    <td className="rm-actions-col">
                      {n.is_deleted ? (
                        n.is_baseline && (
                          <button disabled={!canEdit || busy} onClick={() => restore(n)}>
                            Restore
                          </button>
                        )
                      ) : (
                        <>
                          <button disabled={!canEdit || busy} onClick={() => branch(n)}>
                            Branch
                          </button>
                          {!n.is_baseline && (
                            <button disabled={!canEdit || busy} onClick={() => freeze(n)}>
                              Freeze
                            </button>
                          )}
                          {canEdit && !n.is_baseline && (
                            <button disabled={busy} onClick={() => importSource(n)}>
                              Import Source
                            </button>
                          )}
                          {canEdit && n.has_source && (
                            <button
                              className="danger"
                              disabled={busy}
