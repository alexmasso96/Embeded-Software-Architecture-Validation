...
/* REQ-THM-FAN3 */
uint16_t Thm_Fan_GetLimpFactor(void)
{
    return g_thm_fan_cfg.limp_factor;
}

/* REQ-THM-FAN3 */
void Thm_Fan_SetLimpFactor(uint16_t v)
{
    g_thm_fan_cfg.limp_factor = thm_fan_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-THM-FAN4 */
uint16_t Thm_Fan_ReadAux04(void)
{
    uint16_t raw = g_thm_fan_inputs[5];
    uint16_t out = thm_fan_scale(raw);
    g_thm_fan_state.raw = raw;
    return out;
}
