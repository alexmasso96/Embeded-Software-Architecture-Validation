...
/* REQ-THM-PUM3 */
void Thm_Pump_SetMaxRpm(uint16_t v, uint8_t ramp)
{
    uint16_t lim = thm_pump_clamp_u16(v, g_thm_pump_cfg.max_rpm);
    (void)ramp; /* ramp profile reserved */
    g_thm_pump_cfg.max_rpm = lim;
}
...
{
    return g_thm_pump_cfg.trim_offset;
}

/* REQ-THM-PUM3 */
void Thm_Pump_SetTrimOffset(uint16_t v)
{
    g_thm_pump_cfg.trim_offset = thm_pump_clamp_u16(v, 0xFFFFu);
}

/* REQ-THM-PUM3 */
uint16_t Thm_Pump_GetLimpFactor(void)
{
    return g_thm_pump_cfg.limp_factor;
}

/* REQ-THM-PUM3 */
void Thm_Pump_SetLimpFactor(uint16_t v)
{
    g_thm_pump_cfg.limp_factor = thm_pump_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_thm_pump_inputs[3];
    uint16_t out = thm_pump_scale(raw);
    g_thm_pump_state.raw = raw;
    return out;
}
...
/* REQ-THM-PUM4 */
uint16_t Thm_Pump_ReadAux03(void)
{
    uint16_t raw = g_thm_pump_inputs[4];
    uint16_t out = thm_pump_scale(raw);
    g_thm_pump_state.raw = raw;
    return out;
}
...
/* REQ-THM-PUM4 */
uint16_t Thm_Pump_ReadAux04(void)
{
    uint16_t raw = g_thm_pump_inputs[5];
    uint16_t out = thm_pump_scale(raw);
    g_thm_pump_state.raw = raw;
    return out;
}
...
{
    uint8_t crc = thm_pump_crc8((const uint8_t *)&g_thm_pump_cfg, (uint8_t)sizeof(Thm_Pump_Config_t));
    g_thm_pump_state.valid = (crc != 0u) ? 1u : 0u;
    return g_thm_pump_state.valid;
}
...
void Thm_Pump_Step(void)
{
    ...
    if (Thm_Pump_SelfTest() == 0u) {
        g_thm_pump_state.phase = THM_PUMP_P_LIMP;
    }
    g_thm_pump_state.uptime_ticks++;
}
