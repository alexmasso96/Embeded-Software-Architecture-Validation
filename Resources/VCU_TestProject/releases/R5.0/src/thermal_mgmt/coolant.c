...
/* ============================================================
 * ThermalMgmt :: coolant   (release R5.0)
...
{
    uint16_t lim = thm_coolant_clamp_u16(v, g_thm_coolant_cfg.target_c);
    (void)ramp; /* ramp profile reserved */
    g_thm_coolant_cfg.target_c = lim;
}
...
/* REQ-THM-COO3 */
uint16_t Thm_Coolant_GetLimpFactor(void)
{
    return g_thm_coolant_cfg.limp_factor;
}

/* REQ-THM-COO3 */
void Thm_Coolant_SetLimpFactor(uint16_t v)
{
    g_thm_coolant_cfg.limp_factor = thm_coolant_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_thm_coolant_inputs[5];
    uint16_t out = thm_coolant_scale(raw);
    g_thm_coolant_state.raw = raw;
    return out;
}
...
/* REQ-THM-COO4 */
uint16_t Thm_Coolant_ReadAux04(void)
{
    uint16_t raw = g_thm_coolant_inputs[6];
    uint16_t out = thm_coolant_scale(raw);
    g_thm_coolant_state.raw = raw;
    return out;
}
...
void Thm_Coolant_Step(void)
{
    ...
    g_thm_coolant_state.uptime_ticks++;
}
