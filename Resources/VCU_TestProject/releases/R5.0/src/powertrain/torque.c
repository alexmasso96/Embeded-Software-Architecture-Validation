...
/* ============================================================
 * Powertrain :: torque   (release R5.0)
...
void Pwt_Torque_Init(const Pwt_Torque_Config_t *cfg)
{
    ...
    g_pwt_torque_state.fault_count = 0u;
    pwt_torque_reset();
}
...
{
    uint16_t lim = pwt_torque_clamp_u16(v, g_pwt_torque_cfg.max_torque_nm);
    (void)ramp; /* ramp profile reserved */
    g_pwt_torque_cfg.max_torque_nm = lim;
}
...
{
    return g_pwt_torque_cfg.trim_offset;
}

/* REQ-PWT-TOR3 */
void Pwt_Torque_SetTrimOffset(uint16_t v)
{
    g_pwt_torque_cfg.trim_offset = pwt_torque_clamp_u16(v, 0xFFFFu);
}

/* REQ-PWT-TOR3 */
uint16_t Pwt_Torque_GetLimpFactor(void)
{
    return g_pwt_torque_cfg.limp_factor;
}

/* REQ-PWT-TOR3 */
void Pwt_Torque_SetLimpFactor(uint16_t v)
{
    g_pwt_torque_cfg.limp_factor = pwt_torque_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_pwt_torque_inputs[3];
    uint16_t out = pwt_torque_scale(raw);
    g_pwt_torque_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_pwt_torque_inputs[4];
    uint16_t out = pwt_torque_scale(raw);
    g_pwt_torque_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_pwt_torque_inputs[5];
    uint16_t out = pwt_torque_scale(raw);
    g_pwt_torque_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_pwt_torque_inputs[6];
    uint16_t out = pwt_torque_scale(raw);
    g_pwt_torque_state.raw = raw;
    return out;
}
...
{
    uint8_t crc = pwt_torque_crc8((const uint8_t *)&g_pwt_torque_cfg, (uint8_t)sizeof(Pwt_Torque_Config_t));
    g_pwt_torque_state.valid = (crc != 0u) ? 1u : 0u;
    return g_pwt_torque_state.valid;
}
...
void Pwt_Torque_Step(void)
{
    ...
    if (g_pwt_torque_state.value > g_pwt_torque_cfg.max_torque_nm) {
        g_pwt_torque_state.fault_count++;
        g_pwt_torque_state.phase = PWT_TORQUE_P_FAULT;
    }
    if (Pwt_Torque_SelfTest() == 0u) {
        g_pwt_torque_state.phase = PWT_TORQUE_P_LIMP;
    }
    g_pwt_torque_state.uptime_ticks++;
}
