...
/* ============================================================
 * ChassisControl :: traction   (release R5.0)
...
{
    uint16_t lim = chs_traction_clamp_u16(v, g_chs_traction_cfg.tc_slip_pct);
    (void)ramp; /* ramp profile reserved */
    g_chs_traction_cfg.tc_slip_pct = lim;
}
...
{
    return g_chs_traction_cfg.limp_factor;
}

/* REQ-CHS-TRA3 */
void Chs_Traction_SetLimpFactor(uint16_t v)
{
    g_chs_traction_cfg.limp_factor = chs_traction_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_chs_traction_inputs[4];
    uint16_t out = chs_traction_scale(raw);
    g_chs_traction_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_chs_traction_inputs[5];
    uint16_t out = chs_traction_scale(raw);
    g_chs_traction_state.raw = raw;
    return out;
}
...
void Chs_Traction_Step(void)
{
    ...
    g_chs_traction_state.uptime_ticks++;
}
