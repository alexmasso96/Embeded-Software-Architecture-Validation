...
/* ============================================================
 * ChassisControl :: steering   (release R5.0)
...
/* REQ-CHS-STE3 */
uint16_t Chs_Steering_GetLimpFactor(void)
{
    return g_chs_steering_cfg.limp_factor;
}

/* REQ-CHS-STE3 */
void Chs_Steering_SetLimpFactor(uint16_t v)
{
    g_chs_steering_cfg.limp_factor = chs_steering_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-CHS-STE4 */
uint16_t Chs_Steering_ReadAux04(void)
{
    uint16_t raw = g_chs_steering_inputs[6];
    uint16_t out = chs_steering_scale(raw);
    g_chs_steering_state.raw = raw;
    return out;
}
