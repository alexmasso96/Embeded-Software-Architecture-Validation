...
/* ============================================================
 * ChassisControl :: abs   (release R5.0)
...
{
    uint16_t lim = chs_abs_clamp_u16(v, g_chs_abs_cfg.slip_thresh_pct);
    (void)ramp; /* ramp profile reserved */
    g_chs_abs_cfg.slip_thresh_pct = lim;
}
...
/* REQ-CHS-ABS3 */
uint16_t Chs_Abs_GetLimpFactor(void)
{
    return g_chs_abs_cfg.limp_factor;
}

/* REQ-CHS-ABS3 */
void Chs_Abs_SetLimpFactor(uint16_t v)
{
    g_chs_abs_cfg.limp_factor = chs_abs_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_chs_abs_inputs[6];
    uint16_t out = chs_abs_scale(raw);
    g_chs_abs_state.raw = raw;
    return out;
}
...
/* REQ-CHS-ABS4 */
uint16_t Chs_Abs_ReadAux04(void)
{
    uint16_t raw = g_chs_abs_inputs[7];
    uint16_t out = chs_abs_scale(raw);
    g_chs_abs_state.raw = raw;
    return out;
}
