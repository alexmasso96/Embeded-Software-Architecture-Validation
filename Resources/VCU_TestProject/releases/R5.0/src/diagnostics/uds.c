...
/* REQ-DIAG-UDS3 */
uint16_t Diag_Uds_GetLimpFactor(void)
{
    return g_diag_uds_cfg.limp_factor;
}

/* REQ-DIAG-UDS3 */
void Diag_Uds_SetLimpFactor(uint16_t v)
{
    g_diag_uds_cfg.limp_factor = diag_uds_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-DIAG-UDS4 */
uint16_t Diag_Uds_ReadAux04(void)
{
    uint16_t raw = g_diag_uds_inputs[5];
    uint16_t out = diag_uds_scale(raw);
    g_diag_uds_state.raw = raw;
    return out;
}
