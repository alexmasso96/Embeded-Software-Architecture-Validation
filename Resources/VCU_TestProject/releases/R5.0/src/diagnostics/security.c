...
/* ============================================================
 * Diagnostics :: security   (release R5.0)
...
/* REQ-DIAG-SEC3 */
uint16_t Diag_Security_GetLimpFactor(void)
{
    return g_diag_security_cfg.limp_factor;
}

/* REQ-DIAG-SEC3 */
void Diag_Security_SetLimpFactor(uint16_t v)
{
    g_diag_security_cfg.limp_factor = diag_security_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-DIAG-SEC4 */
uint16_t Diag_Security_ReadAux04(void)
{
    uint16_t raw = g_diag_security_inputs[5];
    uint16_t out = diag_security_scale(raw);
    g_diag_security_state.raw = raw;
    return out;
}
