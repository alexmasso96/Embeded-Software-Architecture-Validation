...
/* ============================================================
 * Diagnostics :: monitor   (release R5.0)
...
/* REQ-DIAG-MON3 */
uint16_t Diag_Monitor_GetLimpFactor(void)
{
    return g_diag_monitor_cfg.limp_factor;
}

/* REQ-DIAG-MON3 */
void Diag_Monitor_SetLimpFactor(uint16_t v)
{
    g_diag_monitor_cfg.limp_factor = diag_monitor_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-DIAG-MON4 */
uint16_t Diag_Monitor_ReadAux04(void)
{
    uint16_t raw = g_diag_monitor_inputs[5];
    uint16_t out = diag_monitor_scale(raw);
    g_diag_monitor_state.raw = raw;
    return out;
}
