...
/* ============================================================
 * Diagnostics :: freezeframe   (release R5.0)
...
void Diag_Freezeframe_Init(const Diag_Freezeframe_Config_t *cfg)
{
    ...
    g_diag_freezeframe_state.fault_count = 0u;
...
/* REQ-DIAG-FRE3 */
void Diag_Freezeframe_SetFrameDepth(uint16_t v, uint8_t ramp)
{
    uint16_t lim = diag_freezeframe_clamp_u16(v, g_diag_freezeframe_cfg.frame_depth);
    (void)ramp; /* ramp profile reserved */
    g_diag_freezeframe_cfg.frame_depth = lim;
}
...
{
    return g_diag_freezeframe_cfg.trim_offset;
}

/* REQ-DIAG-FRE3 */
void Diag_Freezeframe_SetTrimOffset(uint16_t v)
{
    g_diag_freezeframe_cfg.trim_offset = diag_freezeframe_clamp_u16(v, 0xFFFFu);
}

/* REQ-DIAG-FRE3 */
uint16_t Diag_Freezeframe_GetLimpFactor(void)
{
    return g_diag_freezeframe_cfg.limp_factor;
}

/* REQ-DIAG-FRE3 */
void Diag_Freezeframe_SetLimpFactor(uint16_t v)
{
    g_diag_freezeframe_cfg.limp_factor = diag_freezeframe_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_diag_freezeframe_inputs[2];
    uint16_t out = diag_freezeframe_scale(raw);
    g_diag_freezeframe_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_diag_freezeframe_inputs[3];
    uint16_t out = diag_freezeframe_scale(raw);
    g_diag_freezeframe_state.raw = raw;
    return out;
}
...
/* REQ-DIAG-FRE4 */
uint16_t Diag_Freezeframe_ReadAux03(void)
{
    uint16_t raw = g_diag_freezeframe_inputs[4];
    uint16_t out = diag_freezeframe_scale(raw);
    g_diag_freezeframe_state.raw = raw;
    return out;
}
...
/* REQ-DIAG-FRE4 */
uint16_t Diag_Freezeframe_ReadAux04(void)
{
    uint16_t raw = g_diag_freezeframe_inputs[5];
    uint16_t out = diag_freezeframe_scale(raw);
    g_diag_freezeframe_state.raw = raw;
    return out;
}
...
{
    uint8_t crc = diag_freezeframe_crc8((const uint8_t *)&g_diag_freezeframe_cfg, (uint8_t)sizeof(Diag_Freezeframe_Config_t));
    g_diag_freezeframe_state.valid = (crc != 0u) ? 1u : 0u;
    return g_diag_freezeframe_state.valid;
}
...
void Diag_Freezeframe_Step(void)
{
    ...
    if (g_diag_freezeframe_state.value > g_diag_freezeframe_cfg.frame_depth) {
        g_diag_freezeframe_state.fault_count++;
        g_diag_freezeframe_state.phase = DIAG_FREEZEFRAME_P_FAULT;
    }
    if (Diag_Freezeframe_SelfTest() == 0u) {
        g_diag_freezeframe_state.phase = DIAG_FREEZEFRAME_P_LIMP;
    }
    g_diag_freezeframe_state.uptime_ticks++;
}
