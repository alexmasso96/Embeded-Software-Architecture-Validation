...
/* REQ-DIAG-DTC3 */
void Diag_Dtc_SetAgingThreshold(uint16_t v, uint8_t ramp)
{
    uint16_t lim = diag_dtc_clamp_u16(v, g_diag_dtc_cfg.aging_threshold);
    (void)ramp; /* ramp profile reserved */
    g_diag_dtc_cfg.aging_threshold = lim;
}
...
{
    return g_diag_dtc_cfg.trim_offset;
}

/* REQ-DIAG-DTC3 */
void Diag_Dtc_SetTrimOffset(uint16_t v)
{
    g_diag_dtc_cfg.trim_offset = diag_dtc_clamp_u16(v, 0xFFFFu);
}

/* REQ-DIAG-DTC3 */
uint16_t Diag_Dtc_GetLimpFactor(void)
{
    return g_diag_dtc_cfg.limp_factor;
}

/* REQ-DIAG-DTC3 */
void Diag_Dtc_SetLimpFactor(uint16_t v)
{
    g_diag_dtc_cfg.limp_factor = diag_dtc_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_diag_dtc_inputs[3];
    uint16_t out = diag_dtc_scale(raw);
    g_diag_dtc_state.raw = raw;
    return out;
}
...
/* REQ-DIAG-DTC4 */
uint16_t Diag_Dtc_ReadAux03(void)
{
    uint16_t raw = g_diag_dtc_inputs[4];
    uint16_t out = diag_dtc_scale(raw);
    g_diag_dtc_state.raw = raw;
    return out;
}
...
/* REQ-DIAG-DTC4 */
uint16_t Diag_Dtc_ReadAux04(void)
{
    uint16_t raw = g_diag_dtc_inputs[5];
    uint16_t out = diag_dtc_scale(raw);
    g_diag_dtc_state.raw = raw;
    return out;
}
...
{
    uint8_t crc = diag_dtc_crc8((const uint8_t *)&g_diag_dtc_cfg, (uint8_t)sizeof(Diag_Dtc_Config_t));
    g_diag_dtc_state.valid = (crc != 0u) ? 1u : 0u;
    return g_diag_dtc_state.valid;
}
...
void Diag_Dtc_Step(void)
{
    ...
    if (Diag_Dtc_SelfTest() == 0u) {
        g_diag_dtc_state.phase = DIAG_DTC_P_LIMP;
    }
    g_diag_dtc_state.uptime_ticks++;
}
