...
/* ============================================================
 * ChargingCtrl :: sequencer   (release R5.0)
...
{
    uint16_t lim = chg_sequencer_clamp_u16(v, g_chg_sequencer_cfg.state_timeout_ms);
    (void)ramp; /* ramp profile reserved */
    g_chg_sequencer_cfg.state_timeout_ms = lim;
}
...
{
    return g_chg_sequencer_cfg.limp_factor;
}

/* REQ-CHG-SEQ3 */
void Chg_Sequencer_SetLimpFactor(uint16_t v)
{
    g_chg_sequencer_cfg.limp_factor = chg_sequencer_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_chg_sequencer_inputs[4];
    uint16_t out = chg_sequencer_scale(raw);
    g_chg_sequencer_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_chg_sequencer_inputs[5];
    uint16_t out = chg_sequencer_scale(raw);
    g_chg_sequencer_state.raw = raw;
    return out;
}
...
void Chg_Sequencer_Step(void)
{
    ...
    g_chg_sequencer_state.uptime_ticks++;
}
