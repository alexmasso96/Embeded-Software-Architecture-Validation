...
/* ============================================================
 * ChargingCtrl :: pilot   (release R5.0)
...
{
    uint16_t lim = chg_pilot_clamp_u16(v, g_chg_pilot_cfg.duty_to_amps);
    (void)ramp; /* ramp profile reserved */
    g_chg_pilot_cfg.duty_to_amps = lim;
}
...
/* REQ-CHG-PIL3 */
uint16_t Chg_Pilot_GetLimpFactor(void)
{
    return g_chg_pilot_cfg.limp_factor;
}

/* REQ-CHG-PIL3 */
void Chg_Pilot_SetLimpFactor(uint16_t v)
{
    g_chg_pilot_cfg.limp_factor = chg_pilot_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_chg_pilot_inputs[4];
    uint16_t out = chg_pilot_scale(raw);
    g_chg_pilot_state.raw = raw;
    return out;
}
...
/* REQ-CHG-PIL4 */
uint16_t Chg_Pilot_ReadAux04(void)
{
    uint16_t raw = g_chg_pilot_inputs[5];
    uint16_t out = chg_pilot_scale(raw);
    g_chg_pilot_state.raw = raw;
    return out;
}
...
void Chg_Pilot_Step(void)
{
    ...
    g_chg_pilot_state.uptime_ticks++;
}
