...
/* ============================================================
 * ChargingCtrl :: acdc   (release R5.0)
...
{
    uint16_t lim = chg_acdc_clamp_u16(v, g_chg_acdc_cfg.max_amps_ac);
    (void)ramp; /* ramp profile reserved */
    g_chg_acdc_cfg.max_amps_ac = lim;
}
...
/* REQ-CHG-ACD3 */
uint16_t Chg_Acdc_GetLimpFactor(void)
{
    return g_chg_acdc_cfg.limp_factor;
}

/* REQ-CHG-ACD3 */
void Chg_Acdc_SetLimpFactor(uint16_t v)
{
    g_chg_acdc_cfg.limp_factor = chg_acdc_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_chg_acdc_inputs[5];
    uint16_t out = chg_acdc_scale(raw);
    g_chg_acdc_state.raw = raw;
    return out;
}
...
/* REQ-CHG-ACD4 */
uint16_t Chg_Acdc_ReadAux04(void)
{
    uint16_t raw = g_chg_acdc_inputs[6];
    uint16_t out = chg_acdc_scale(raw);
    g_chg_acdc_state.raw = raw;
    return out;
}
