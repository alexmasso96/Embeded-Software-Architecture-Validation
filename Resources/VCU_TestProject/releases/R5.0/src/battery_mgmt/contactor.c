...
/* ============================================================
 * BatteryMgmt :: contactor   (release R5.0)
...
{
    uint16_t lim = bms_contactor_clamp_u16(v, g_bms_contactor_cfg.precharge_ms);
    (void)ramp; /* ramp profile reserved */
    g_bms_contactor_cfg.precharge_ms = lim;
}
...
{
    return g_bms_contactor_cfg.limp_factor;
}

/* REQ-BMS-CON3 */
void Bms_Contactor_SetLimpFactor(uint16_t v)
{
    g_bms_contactor_cfg.limp_factor = bms_contactor_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_bms_contactor_inputs[5];
    uint16_t out = bms_contactor_scale(raw);
    g_bms_contactor_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_bms_contactor_inputs[6];
    uint16_t out = bms_contactor_scale(raw);
    g_bms_contactor_state.raw = raw;
    return out;
}
...
void Bms_Contactor_Step(void)
{
    ...
    g_bms_contactor_state.uptime_ticks++;
}
