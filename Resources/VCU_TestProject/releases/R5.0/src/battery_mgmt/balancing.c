...
/* ============================================================
 * BatteryMgmt :: balancing   (release R5.0)
...
/* REQ-BMS-BAL3 */
void Bms_Balancing_SetBalanceDeltaMv(uint16_t v, uint8_t ramp)
{
    uint16_t lim = bms_balancing_clamp_u16(v, g_bms_balancing_cfg.balance_delta_mv);
    (void)ramp; /* ramp profile reserved */
    g_bms_balancing_cfg.balance_delta_mv = lim;
}
...
/* REQ-BMS-BAL3 */
uint16_t Bms_Balancing_GetLimpFactor(void)
{
    return g_bms_balancing_cfg.limp_factor;
}

/* REQ-BMS-BAL3 */
void Bms_Balancing_SetLimpFactor(uint16_t v)
{
    g_bms_balancing_cfg.limp_factor = bms_balancing_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-BMS-BAL4 */
uint16_t Bms_Balancing_ReadAux03(void)
{
    uint16_t raw = g_bms_balancing_inputs[4];
    uint16_t out = bms_balancing_scale(raw);
    g_bms_balancing_state.raw = raw;
    return out;
}
...
/* REQ-BMS-BAL4 */
uint16_t Bms_Balancing_ReadAux04(void)
{
    uint16_t raw = g_bms_balancing_inputs[5];
    uint16_t out = bms_balancing_scale(raw);
    g_bms_balancing_state.raw = raw;
    return out;
}
...
void Bms_Balancing_Step(void)
{
    ...
    g_bms_balancing_state.uptime_ticks++;
}
