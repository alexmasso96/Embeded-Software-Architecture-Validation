...
/* ============================================================
 * BatteryMgmt :: thermrunaway   (release R5.0)
...
{
    return g_bms_thermrunaway_cfg.limp_factor;
}

/* REQ-BMS-THE3 */
void Bms_Thermrunaway_SetLimpFactor(uint16_t v)
{
    g_bms_thermrunaway_cfg.limp_factor = bms_thermrunaway_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_bms_thermrunaway_inputs[5];
    uint16_t out = bms_thermrunaway_scale(raw);
    g_bms_thermrunaway_state.raw = raw;
    return out;
}
