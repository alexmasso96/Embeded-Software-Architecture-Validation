...
/* ============================================================
 * BatteryMgmt :: cellmon   (release R5.0)
...
{
    return g_bms_cellmon_cfg.limp_factor;
}

/* REQ-BMS-CEL3 */
void Bms_Cellmon_SetLimpFactor(uint16_t v)
{
    g_bms_cellmon_cfg.limp_factor = bms_cellmon_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_bms_cellmon_inputs[6];
    uint16_t out = bms_cellmon_scale(raw);
    g_bms_cellmon_state.raw = raw;
    return out;
}
