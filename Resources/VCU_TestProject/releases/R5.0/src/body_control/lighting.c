...
/* ============================================================
 * BodyControl :: lighting   (release R5.0)
...
/* REQ-BCM-LIG3 */
uint16_t Bcm_Lighting_GetLimpFactor(void)
{
    return g_bcm_lighting_cfg.limp_factor;
}

/* REQ-BCM-LIG3 */
void Bcm_Lighting_SetLimpFactor(uint16_t v)
{
    g_bcm_lighting_cfg.limp_factor = bcm_lighting_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-BCM-LIG4 */
uint16_t Bcm_Lighting_ReadAux04(void)
{
    uint16_t raw = g_bcm_lighting_inputs[6];
    uint16_t out = bcm_lighting_scale(raw);
    g_bcm_lighting_state.raw = raw;
    return out;
}
