...
/* ============================================================
 * BodyControl :: wipers   (release R5.0)
...
/* REQ-BCM-WIP3 */
uint16_t Bcm_Wipers_GetLimpFactor(void)
{
    return g_bcm_wipers_cfg.limp_factor;
}

/* REQ-BCM-WIP3 */
void Bcm_Wipers_SetLimpFactor(uint16_t v)
{
    g_bcm_wipers_cfg.limp_factor = bcm_wipers_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-BCM-WIP4 */
uint16_t Bcm_Wipers_ReadAux04(void)
{
    uint16_t raw = g_bcm_wipers_inputs[6];
    uint16_t out = bcm_wipers_scale(raw);
    g_bcm_wipers_state.raw = raw;
    return out;
}
