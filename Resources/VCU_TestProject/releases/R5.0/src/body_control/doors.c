...
/* ============================================================
 * BodyControl :: doors   (release R5.0)
...
{
    uint16_t lim = bcm_doors_clamp_u16(v, g_bcm_doors_cfg.lock_timeout_ms);
    (void)ramp; /* ramp profile reserved */
    g_bcm_doors_cfg.lock_timeout_ms = lim;
}
...
{
    return g_bcm_doors_cfg.limp_factor;
}

/* REQ-BCM-DOO3 */
void Bcm_Doors_SetLimpFactor(uint16_t v)
{
    g_bcm_doors_cfg.limp_factor = bcm_doors_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_bcm_doors_inputs[5];
    uint16_t out = bcm_doors_scale(raw);
    g_bcm_doors_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_bcm_doors_inputs[6];
    uint16_t out = bcm_doors_scale(raw);
    g_bcm_doors_state.raw = raw;
    return out;
}
...
void Bcm_Doors_Step(void)
{
    ...
    g_bcm_doors_state.uptime_ticks++;
}
