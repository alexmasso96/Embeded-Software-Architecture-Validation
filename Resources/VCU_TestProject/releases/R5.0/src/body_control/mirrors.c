...
/* ============================================================
 * BodyControl :: mirrors   (release R5.0)
...
{
    uint16_t lim = bcm_mirrors_clamp_u16(v, g_bcm_mirrors_cfg.fold_angle);
    (void)ramp; /* ramp profile reserved */
    g_bcm_mirrors_cfg.fold_angle = lim;
}
...
{
    return g_bcm_mirrors_cfg.trim_offset;
}

/* REQ-BCM-MIR3 */
void Bcm_Mirrors_SetTrimOffset(uint16_t v)
{
    g_bcm_mirrors_cfg.trim_offset = bcm_mirrors_clamp_u16(v, 0xFFFFu);
}

/* REQ-BCM-MIR3 */
uint16_t Bcm_Mirrors_GetLimpFactor(void)
{
    return g_bcm_mirrors_cfg.limp_factor;
}

/* REQ-BCM-MIR3 */
void Bcm_Mirrors_SetLimpFactor(uint16_t v)
{
    g_bcm_mirrors_cfg.limp_factor = bcm_mirrors_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_bcm_mirrors_inputs[3];
    uint16_t out = bcm_mirrors_scale(raw);
    g_bcm_mirrors_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_bcm_mirrors_inputs[4];
    uint16_t out = bcm_mirrors_scale(raw);
    g_bcm_mirrors_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_bcm_mirrors_inputs[5];
    uint16_t out = bcm_mirrors_scale(raw);
    g_bcm_mirrors_state.raw = raw;
    return out;
}
...
{
    uint8_t crc = bcm_mirrors_crc8((const uint8_t *)&g_bcm_mirrors_cfg, (uint8_t)sizeof(Bcm_Mirrors_Config_t));
    g_bcm_mirrors_state.valid = (crc != 0u) ? 1u : 0u;
    return g_bcm_mirrors_state.valid;
}
...
void Bcm_Mirrors_Step(void)
{
    ...
    if (Bcm_Mirrors_SelfTest() == 0u) {
        g_bcm_mirrors_state.phase = BCM_MIRRORS_P_LIMP;
    }
    g_bcm_mirrors_state.uptime_ticks++;
}
