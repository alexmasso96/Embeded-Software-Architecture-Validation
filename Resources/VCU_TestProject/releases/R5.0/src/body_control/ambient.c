...
/* ============================================================
 * BodyControl :: ambient   (release R5.0)
...
/* REQ-BCM-AMB3 */
uint16_t Bcm_Ambient_GetLimpFactor(void)
{
    return g_bcm_ambient_cfg.limp_factor;
}

/* REQ-BCM-AMB3 */
void Bcm_Ambient_SetLimpFactor(uint16_t v)
{
    g_bcm_ambient_cfg.limp_factor = bcm_ambient_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-BCM-AMB4 */
uint16_t Bcm_Ambient_ReadAux04(void)
{
    uint16_t raw = g_bcm_ambient_inputs[5];
    uint16_t out = bcm_ambient_scale(raw);
    g_bcm_ambient_state.raw = raw;
    return out;
}
