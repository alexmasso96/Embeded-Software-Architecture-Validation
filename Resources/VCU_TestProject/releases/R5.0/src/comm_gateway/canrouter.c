...
/* ============================================================
 * CommGateway :: canrouter   (release R5.0)
...
/* REQ-COM-CAN3 */
uint16_t Com_Canrouter_GetLimpFactor(void)
{
    return g_com_canrouter_cfg.limp_factor;
}

/* REQ-COM-CAN3 */
void Com_Canrouter_SetLimpFactor(uint16_t v)
{
    g_com_canrouter_cfg.limp_factor = com_canrouter_clamp_u16(v, 0xFFFFu);
}
...
/* REQ-COM-CAN4 */
uint16_t Com_Canrouter_ReadAux04(void)
{
    uint16_t raw = g_com_canrouter_inputs[6];
    uint16_t out = com_canrouter_scale(raw);
    g_com_canrouter_state.raw = raw;
    return out;
}
