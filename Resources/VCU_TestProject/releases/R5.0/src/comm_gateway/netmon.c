...
/* ============================================================
 * CommGateway :: netmon   (release R5.0)
...
{
    return g_com_netmon_cfg.limp_factor;
}

/* REQ-COM-NET3 */
void Com_Netmon_SetLimpFactor(uint16_t v)
{
    g_com_netmon_cfg.limp_factor = com_netmon_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_com_netmon_inputs[5];
    uint16_t out = com_netmon_scale(raw);
    g_com_netmon_state.raw = raw;
    return out;
}
