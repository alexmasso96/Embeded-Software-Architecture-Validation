...
/* ============================================================
 * CommGateway :: signaldb   (release R5.0)
...
void Com_Signaldb_Init(const Com_Signaldb_Config_t *cfg)
{
    ...
    g_com_signaldb_state.fault_count = 0u;
    com_signaldb_reset();
}
...
/* REQ-COM-SIG3 */
void Com_Signaldb_SetTimeoutMs(uint16_t v, uint8_t ramp)
{
    uint16_t lim = com_signaldb_clamp_u16(v, g_com_signaldb_cfg.timeout_ms);
    (void)ramp; /* ramp profile reserved */
    g_com_signaldb_cfg.timeout_ms = lim;
}
...
{
    return g_com_signaldb_cfg.trim_offset;
}

/* REQ-COM-SIG3 */
void Com_Signaldb_SetTrimOffset(uint16_t v)
{
    g_com_signaldb_cfg.trim_offset = com_signaldb_clamp_u16(v, 0xFFFFu);
}

/* REQ-COM-SIG3 */
uint16_t Com_Signaldb_GetLimpFactor(void)
{
    return g_com_signaldb_cfg.limp_factor;
}

/* REQ-COM-SIG3 */
void Com_Signaldb_SetLimpFactor(uint16_t v)
{
    g_com_signaldb_cfg.limp_factor = com_signaldb_clamp_u16(v, 0xFFFFu);
}
...
{
    uint16_t raw = g_com_signaldb_inputs[3];
    uint16_t out = com_signaldb_scale(raw);
    g_com_signaldb_state.raw = raw;
    return out;
}
...
{
    uint16_t raw = g_com_signaldb_inputs[4];
    uint16_t out = com_signaldb_scale(raw);
    g_com_signaldb_state.raw = raw;
    return out;
}
...
/* REQ-COM-SIG4 */
uint16_t Com_Signaldb_ReadAux03(void)
{
    uint16_t raw = g_com_signaldb_inputs[5];
    uint16_t out = com_signaldb_scale(raw);
    g_com_signaldb_state.raw = raw;
    return out;
}
...
/* REQ-COM-SIG4 */
uint16_t Com_Signaldb_ReadAux04(void)
{
    uint16_t raw = g_com_signaldb_inputs[6];
    uint16_t out = com_signaldb_scale(raw);
    g_com_signaldb_state.raw = raw;
    return out;
}
...
{
    uint8_t crc = com_signaldb_crc8((const uint8_t *)&g_com_signaldb_cfg, (uint8_t)sizeof(Com_Signaldb_Config_t));
    g_com_signaldb_state.valid = (crc != 0u) ? 1u : 0u;
    return g_com_signaldb_state.valid;
}
...
void Com_Signaldb_Step(void)
{
    ...
    if (g_com_signaldb_state.value > g_com_signaldb_cfg.timeout_ms) {
        g_com_signaldb_state.fault_count++;
        g_com_signaldb_state.phase = COM_SIGNALDB_P_FAULT;
    }
    if (Com_Signaldb_SelfTest() == 0u) {
        g_com_signaldb_state.phase = COM_SIGNALDB_P_LIMP;
    }
    g_com_signaldb_state.uptime_ticks++;
}
