...
Phase 0 (pywebview migration): Qt-free. All user interaction goes through the
controller-provided callbacks (notify_*, ask_*, busy — see
UI/controller_feedback.py for the Qt implementation).
"""
import json
import logging
...
class ArchitectureBaselineMixin:
    ...
    def handle_create_baseline(self):
        ...
        if active_idx == -1:
            self.notify_warning("Warning", "No active release to baseline.")
            return

        active_release = self.release_manager.get_active_release()
        if active_release.is_baseline:
            self.notify_warning(
                "Warning", "Cannot create a baseline from a baseline."
            )
            return

        name = self.ask_text("Create Baseline", "Enter Baseline Name:")
        ...
        try:
            ...
            self.notify_info("Success", f"Created Baseline: {baseline.name}")

        except Exception as e:
            self.notify_error("Error", f"Failed to create baseline: {e}")
    ...
    def handle_load_baseline(self):
        ...
        if not baselines:
            self.notify_info("Load Baseline", "No baselines available to load.")
            return

        name = self.ask_choice(
            "Load Baseline", "Select a baseline to load:",
            [b.name for b in baselines]
        )
        if not name:
            return
    ...
    def load_baseline_by_model(self, selected_baseline):
        if ProjectSaver.has_temp_changes(self.release_manager.project_path):
            ...
            if reply == "yes":
                ...
                if not success:
                    self.notify_error("Save Error", f"Could not save project: {msg}")
                    return
            elif reply == "no":
        ...
        try:
            with self.busy(f"Loading baseline {selected_baseline.name}..."):
    ...
    def handle_exit_baseline(self):
        ...
        try:
            with self.busy("Exiting Baseline View, restoring project..."):
            ...
            self.notify_error("Error", f"Failed to exit baseline view: {e}")
        finally:
            self.table.setVisible(True)
