"""
Advanced AI Chat — Tab 4 controller (Qt, Phases 11 + 12).

Moved out of Application_Logic/Logic_AI_Chat.py in Phase 0 of the pywebview
migration; the mind-map/chat jobs and markdown rendering live there as pure
functions. A 2-column tab: left = source releases (synced with Tab 3),
requirements import, mind-map generation/diffs, separate prompt/rules editors,
and the shared provider panel; right = an agentic chat grounded in the mind
map + read-only code tools (Phase 9). Token strategy (Phase 12): the compact
mind map is the default context injected per turn — raw C is reached only on
demand via tools.

UI is manual-tested; the heavy logic lives in Logic_AI_Context /
Logic_AI_Providers / Logic_AI_Tools / Logic_AI_Chat (unit-tested).
"""
...
from Application_Logic.Logic_AI_Chat import (
    _META_LAST_DIFF, md_to_html, run_mindmap_job, run_chat_job,
)
from UI.provider_panel import ProviderPanelMixin

logger = logging.getLogger(__name__)


class _MindMapWorker(QtCore.QThread):
    """Thin Qt thread around the pure ``run_mindmap_job``."""
    ...
    def run(self):
        try:
            n = run_mindmap_job(self.jobs, self.current_source,
                                self.previous_source, self.db, self.release_id,
...
class _ChatWorker(QtCore.QThread):
    """Thin Qt thread around the pure ``run_chat_job``."""
    ...
    def run(self):
        try:
            out = run_chat_job(
                ...
                self.executor, self.system_prompt,
