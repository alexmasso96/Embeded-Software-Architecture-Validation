"""
AI Test Case Generation — tab controller (Qt).

Moved out of Application_Logic/Logic_AI_Generation.py in Phase 0 of the
pywebview migration; the generation job lives there as a pure function.
...
lives in Logic_AI_Context / Logic_AI_Providers / Logic_AI_Generation (unit-tested).
"""
from PyQt6 import QtWidgets, QtCore

from Application_Logic import Logic_AI_Providers as providers
from Application_Logic import Logic_AI_Context as ctx
from Application_Logic.Logic_AI_Generation import (
    _META_PROVIDER, _META_MODEL, run_generation_job,
)
from UI.provider_panel import ProviderPanelMixin


class _GenWorker(QtCore.QThread):
    """Thin Qt thread around the pure ``run_generation_job`` — relays its
    callbacks as queued signals onto the GUI thread."""
    ...
    def run(self):
        try:
            path = run_generation_job(
                self.provider_id, self.model, self.rules, self.prompt,
                self.source_path, self.output_dir, self.model_name,
                self.hlt_title, self.test_cases,
                progress_cb=self.progress.emit,
                case_done_cb=self.case_done.emit,
