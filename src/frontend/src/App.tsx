...
export default function App() {
  ...
  // Pending Code Map focus, set when the user picks "Show in Code Map" from the
  // Workspace; consumed by the CodeMap view once it mounts.
  const [codeMapFocus, setCodeMapFocus] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
  const [prefsOpen, setPrefsOpen] = useState(false);
  const [importOpen, setImportOpen] = useState(false);
  const [columnsOpen, setColumnsOpen] = useState(false);
  const [viewBannerDismissed, setViewBannerDismissed] = useState(false);
  const [changelogOpen, setChangelogOpen] = useState(false);
  const { update, check: checkUpdates } = useUpdateCheck();
  ...
  return (
    <div className="app">
      ...
      {!status.lock_lost && status.mode === "view" && !viewBannerDismissed && (
        <div className="banner warn">
          ...
          <button
            ...
            title="Dismiss"
            onClick={() => setViewBannerDismissed(true)}
          >
            ✕
          </button>
        </div>
      ...
      {activeTab === "Workspace" ? (
        ...
        <CodeMap
          toast={toast}
          focusTarget={codeMapFocus}
          onFocusConsumed={() => setCodeMapFocus(null)}
        />
