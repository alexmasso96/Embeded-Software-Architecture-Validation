...
import { getFileExplorer } from "../settings";
import { isDesktop, nativePickFile, nativePickFolder, nativeSaveFile } from "../native";
...
export function FolderPicker({
  ...
  // File-explorer preference (issue 6): when the user picks "system" dialogs and
  // we're running in the desktop shell, hand straight off to the native OS
  // chooser instead of rendering the in-app browser. In a plain browser there's
  // no native bridge, so we fall through to the custom picker regardless.
  const useSystemDialog = getFileExplorer() === "system" && isDesktop();
  const [bypassing] = useState(useSystemDialog);
  useEffect(() => {
    if (!useSystemDialog) return;
    let cancelled = false;
    (async () => {
      try {
        ...
        if (mode === "folder") {
          ...
          path = await nativeSaveFile("Untitled.arch");
          // The native Save dialog may return a name without our extension.
          if (path && !path.toLowerCase().endsWith(".arch")) path += ".arch";
        } else {
          path = await nativePickFile(exts ?? (mode === "import" ? [".elf", ".json"] : [".arch"]));
        }
        if (cancelled) return;
        // Native open has no view/edit toggle — match the custom picker's
        // default of view-only; the user can re-open for editing as before.
        if (path) onConfirm(path, "view");
        else onCancel();
      } catch {
  ...
  // Native OS dialog in flight — don't flash the in-app browser behind it.
