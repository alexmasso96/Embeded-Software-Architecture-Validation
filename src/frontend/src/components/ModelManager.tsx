...
export function ModelManager({
  ...
  function setStatus(status: string) {
    ...
    // Strict forward: only In Work → Released cascades onto the model's still-
    // "In Work" ports. Preview first; if any port is affected, let the user
    // confirm/select which ones follow before committing. Otherwise fall through
    // to a plain status change. (The <select> stays bound to model.status, so a
    // cancel naturally snaps the control back to the previous value.)
    if (model.status === "In Work" && status === "Released") {
      run(async () => {
        try {
  ...
  // Empty models (no ports) are removed permanently with no soft-delete entry;
  // non-empty models soft-delete (and stay restorable) after a themed confirm.
  function deleteModel() {
    ...
    confirm({
      ...
      message: (
        <>
          Delete model <b>“{model.name}”</b>? It has {model.row_count} port
          {model.row_count === 1 ? "" : "s"} and can be restored later.
        </>
      ),
      confirmLabel: "Delete",
      danger: true,
      ...
      run(async () => {
        try {
          await api.patch(`/models/${model.id}`, { is_deleted: true });
          await reload();
        } catch (e) {
          toast(`Delete failed: ${(e as Error).message}`);
        }
            ...
            <div className="mm-actions">
              ...
              {sel?.is_deleted ? (
                ...
                <button
                  ...
                  title={
                    sel?.row_count === 0
                      ? "Permanently delete this empty model"
                      : "Soft-delete this model (restorable)"
                  }
                >
                  {sel?.row_count === 0 ? "Delete" : "Delete…"}
                </button>
              )}
            </div>
          </div>
