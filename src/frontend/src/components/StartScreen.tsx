...
// Small status pill pinned to the bottom-right of the launcher (issue 1).
// Green when up to date; yellow/amber while checking, on error, or when an
// update is available — clicking the available pill opens the changelog.
function UpdatePill({
  update,
  onShowChangelog,
}: {
  ...
  if (update.status === "update-available") {
    return (
      <button
        className="update-pill warn"
        title={`v${update.latestVersion} is available — view changelog`}
  ...
  if (update.status === "latest") {
    return (
      <div className="update-pill ok" title="You're on the latest version">
        <span className="update-pill-dot" />
        Up to date
      </div>
    );
  ...
  if (update.status === "error") {
    return (
      <div className="update-pill warn" title="Offline or update check failed">
        <span className="update-pill-dot" />
        Update check failed
      </div>
    );
  ...
  // idle / checking
  return (
    <div className="update-pill warn" title="Contacting GitHub…">
      <span className="spin" />
...
export function StartScreen({
  ...
  return (
    <div className="launcher">
      <UpdatePill update={update} onShowChangelog={onShowChangelog} />
      <div className="start">
