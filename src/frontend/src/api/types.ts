...
export interface ProjectStatus {
  ...
  active_release_is_baseline?: boolean;
  model_count: number;
...
// Per ReleaseResultColumn: which release it tracks and the flags that drive
// value derivation + edit-gating (set by the backend).
...
export interface ColumnsResponse {
  columns: ColumnSpec[];
  result_columns?: ResultColumnMeta[];
}
...
// ASPICE change-history log. Read-only, newest-first entries for the active
// release. Model History shows them all; Port History filters to one port.
